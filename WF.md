...
**Engine-state janitor.** The FSM is the source of truth; `temporal.db` is re-derivable, so the housekeeping sweep (every 5 minutes, per project) reconciles drift between them in `sweepZombieWorkflows` (`server/internal/api/dispatch_engine.go`). It walks the workflow-ID families — `run`/`merge`/`resolve`/`addcommit` (scope: workstream), `plan`/`edit`/`plansplit` (scope: story), `docs`/`discovery`/`detect`/`pmdescribe` (scope: project-transient); the `pm:`/`mergelock:` singletons are excluded because cancelling the parent workflows covers them. Two passes per family: an OPEN pass cancels a still-running execution whose entity is FSM-terminal or absent, and a CLOSED pass deletes the history of a closed execution for a terminal/absent entity the way a merged PR's branch is pruned — leaving the 7-day namespace retention as the backstop for everything else (failed runs an operator may still inspect). Project-transient families carry no per-entity FSM row, so they cancel only past a 72h age floor and skip the closed pass entirely; `docs`/`discovery` are singleton IDs matched exactly (not by prefix) so a sweep of `proj` cannot reach `proj-staging`. A non-`sql.ErrNoRows` lookup error fails safe (entity treated as live). Cancellation is a request, not a kill: the workflow-side signal waits (the run workflow's human-gate park, the artifact-lock grant wait) select over cancellation so a cancelled parked workflow exits as CANCELED instead of blocking in a bare `Receive` forever. Should an execution still be RUNNING with a cancel already pending on a later sweep (`DescribeWorkflowExecution` extended info), the OPEN pass escalates to terminate rather than re-cancelling every 5 minutes to no effect. Every action writes a durable `engine_cleanup` events-table row carrying the workflow id, run id, action, and the reason the entity qualified (`workstream X is merged`, `story Y no longer exists`, ...). Cancel/abandon/remove are themselves recorded operator intent, so terminal status is the authorization; workstream removal additionally cancels its own open executions inline and, when a PR is attached, closes it on the forge and deletes the remote branch (idempotent when the ref is already gone), with the janitor as the backstop for anything missed. A janitor acting on the same target every consecutive sweep is a loop, not convergence: after 12 consecutive sweeps (~1 hour) the sweep emits one project-scoped `engine_loop_detected` event naming the target, the action, the streak count, and its first occurrence, then stays quiet until the loop breaks (streak resets in-memory when a sweep skips the target, or on server restart) or the streak doubles.

**Engine-health incidents.** The server's own engine calls feed a debounced incident tracker (`server/internal/enginehealth`): the `workflowOpen` liveness funnel and the janitor's visibility lists report success/failure directly, and a Temporal SDK logger adapter (installed at the in-server client dial) reports the worker's infra-shaped log entries (task-processing / poll failures). Two failures inside two minutes open an incident and emit one system-scoped `engine_degraded` event; the first success at least a minute after the last failure closes it with one `engine_recovered` carrying the outage span and per-source failure counts. System-scoped events have no project: the durable rows live in the ops-root events store, and the SSE bridge broadcasts them to every subscriber regardless of project filter. Operator surfaces: the TUI footer shows `engine degraded` while an incident is open, and `hashd status` / `hashd doctor` list the last 7 days of incident events. Separately, a SIGTERM->SIGKILL escalation on an agent subprocess emits a project-scoped `agent_killed` event (workstream, stage, pid, and whether the run was cancelled or timed out) via the internal `/agent-invocations/killed` observability post.
...
2. **Across attempts** — each long agent stage activity carries a `RetryPolicy` (`agentActivityOptions`: `InitialInterval 5s`, `BackoffCoefficient 2.0`, `MaximumInterval 60s`) plus a 120s `HeartbeatTimeout`. It retries ONLY infrastructure failures (worker death detected via a missed heartbeat, DB faults). An agent *outcome* is returned as data with a nil error and never retries — the outcome/error split is the guard, so a review verdict of "changes required" is data, not a retryable error. Tier A stages whose whole-body re-run is crash-safe (implement, adjudicate, concern_triage, test, probe, detect, provision, final_review, and review -- its agent-outcome errors carry the typed non-retryable `review_outcome` marker so only infra shapes retry) use `MaximumAttempts>1`; stages that persist a plan or transition mid-body (breakdown, partial_breakdown) and the one-shot commit leg keep the heartbeat but stay `MaximumAttempts:1`. On a Tier-A retry the implement driver resumes the agent's session (crash-resume); other stages re-run fresh. **Cancellation is a different layer from an agent outcome.** When the activity context is cancelled -- worker shutdown, heartbeat-timeout invalidation, a persistence brownout -- the runner kills the agent subprocess off `ctx.Done` and the killed attempt classifies `interrupted` (the holistic judges surface the same kill as an `Unavailable` / error verdict). That is the box blinking *before* the agent finished, not the agent's answer. So a **single boundary helper, `propagateCancel`, wraps the return of every agent-stage activity** (implement, test, review, final_review, adjudicate, concern_triage, breakdown, partial_breakdown, the committed-breakage probe, provision, detect, scope_adjudicate): when the context is cancelled it discards whatever the driver returned -- a nil-error `interrupted` / `Unavailable` / `Verdict:"error"` / `"Build failed"` outcome, or a wrapped context error -- and returns the bare `ctx.Err()`. This is one authority rather than a guard per stage, so no stage -- present or future -- can silently masquerade a cancellation as an outcome. Temporal then splits the two cancellation kinds by cause: an infrastructure-driven cancel (worker death / heartbeat-timeout / brownout, a non-server cause) is a retryable FAILED, so the RetryPolicy fires and the stage re-runs (implement resumes its session; the judge rules on the next attempt) instead of the run closing `interrupted` or parking a human on a "could not run" no one asked for yet; a **server-requested cancel (operator stop) carries a `CanceledError` cause, so the SDK reports it terminal-canceled and the workflow's cancellation path handles it -- no retry**. Returning the bare `ctx.Err()` -- not a wrapped or typed error -- is what lets the SDK's `cancelAllowed` check make that split. The fail-closed path is **not** weakened -- it is reached truthfully. A judge that *ran* and could not settle (its agent timed out, was rejected, exhausted context, or self-exited, with the context still alive) still renders `could not run (<reason>)` and routes to a human; and so does attempt exhaustion, which closes the run `failed` with an operator note naming the in-flight stage and micro-commit and pointing at `hashd run` -- the abandoned attempt surfaced, not left as a stale pre-kill reading. The discriminator everywhere is `ctx.Err()`: non-nil means the box blinked (propagate -- Temporal retries an infra cancel, terminates a server-requested one); nil means the agent produced the outcome (fail-closed, no retry).
